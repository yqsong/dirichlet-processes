...
//import index.PrepareBankTimeCorpora;
//
//import java.io.File;
...
//
//public class HDPTopicSplitMerge {
...
//	 */
//	List<List<OpenIntIntHashMap>> tableNumbers;
//	
//	/**
...
//	/**
//	 * Private constructor.
//	 * @param docTimeCorpora, the docTimeCorpora structure containing the feature sequences of all
//	 * documents
...
//			for(int d = 0; d < this.docTimeCorpora.docNumbers.get(j); d++){
//				this.Z.get(j).add(new int[this.docTimeCorpora.docLengths.get(j).get(d)]);
//			}
//		}
//		
...
//		}
//		this.newTopicLike = 1.0 / this.docTimeCorpora.dictSize;
//		this.b0TimesW = this.b0 * this.docTimeCorpora.dictSize;
...
//					(this.docTimeCorpora.docNumbers.get(j) < max_sel)? this.docTimeCorpora.docNumbers.get(j) : max_sel);
//			this.cvgDocs.add(new ArrayList<Integer>());
//			int[] rpm = this.randomSampler.RandPerm(this.docTimeCorpora.docNumbers.get(j));
...
//		}
//		for(int j = 0; j < this.docTimeCorpora.corporaNumber; j++){
//			int docnum = this.docTimeCorpora.docNumbers.get(j);
...
//			for(int d = 0; d < docnum; d++){
//				initlabel.get(j).add(this.randInitLabel(this.docTimeCorpora.docLengths.get(j).get(d), alphabet));
//			}
//		}
...
//					wjdi = this.docTimeCorpora.ftSequences.get(j).get(d)[i];
...
//				L = this.docTimeCorpora.docLengths.get(j).get(d);
//				for(int i = 0; i < L; i++){
//					w = this.docTimeCorpora.ftSequences.get(j).get(d)[i];
...
//		for(int j = 0; j < J; j++){
//		
//			int D = this.docTimeCorpora.docNumbers.get(j);
//			List<Integer> nd = this.docTimeCorpora.docLengths.get(j);
...
//			metaTableNum[0] = 0;
//			betas = new double[this.docTimeCorpora.corporaNumber][K + 1];
//			for(int j = 0; j < this.docTimeCorpora.corporaNumber; j++){
...
//					this.docTimeCorpora.dictionary
//					, this.docTimeCorpora.forwardHash
//					, this.docTimeCorpora.backwardHash
//					, this.docTimeCorpora.ftSequences
//					, this.Z
//					, this.docTopicSize
//					, this.topicTermFreq
...
//		int[][] ret = new int[J][];
//		for(int j = 0; j < J; j++){
//			ret[j] = new int[this.docTimeCorpora.docNumbers.get(j)];
//			for(int d = 0; d < this.docTimeCorpora.docNumbers.get(j); d++){
...
//				d = docs.get(j).get(rd);
//				for(int i = 0; i < this.docTimeCorpora.ftSequences.get(j).get(d).length; i++){
//					w = this.docTimeCorpora.ftSequences.get(j).get(d)[i];
...
////				words[i] = (String) this.docTimeCorpora.dictionary.lookupObject(wids.get(maxN[i]));
...
////		String outputFolder = "C:\\data_test\\cobra_bank\\output";
//
...
//			
//		DocTimeCorpora docTimeCorpora = prepareTimeData.initializeData (luceneSrc, queryStr, false, 1, 1, stopwordFile);
//
//		double value = 0.5;
//		double b0 = value;
//		double gamma = value, alpha = value;
//		double gamma_a = value, gamma_b = value * 20;
//		double alpha_a = value, alpha_b = value * 20;
//		
